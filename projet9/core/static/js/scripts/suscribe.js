            ...
            success(json){
                if(json.status == 1){
                    $('.followed-users').append(
                     '<div class="row p-2 col-12 border color-swap"><p class="col-xl-10 col-md-10 col-sm-8">'+ json.username +'</p><button class="btn btn-secondary col-xl-2 col-md-2 col-sm-4">Supprimer</button></div>'
                    )
                    $('.color-swap:odd').addClass('color-swap-1')
                    $('.color-swap:odd>button.btn').addClass('btn-light')
                    $('.color-swap:even').addClass('color-swap-2')
