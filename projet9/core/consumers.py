...
class LoginConsumers(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if User.objects.filter(username__iexact=username, password__iexact=password).exists():
            user = await User.objects.get(username__iexact=username, password__iexact=password)
